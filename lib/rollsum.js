...
function Rollsum(chunkSize, digestSize, options) {
  if(chunkSize<=0 || digestSize <=0){
    throw new Error("Invalid arguments for rollsum, chunkSize and digestSize must be positive");
  }
  Writable.call(this, options);
  this.chunkSize = chunkSize;
  this.digestSize = digestSize;
  this.digestBuffer = new Buffer(digestSize*4);
  this.buffer = new Buffer(chunkSize + digestSize - 1);
  this.rollsum = new rollsum.RollsumObj(this.buffer, chunkSize, this.digestBuffer, digestSize);
  var $this = this;
...
Rollsum.prototype.getDigest = function(i){
  var b = this.digestBuffer;
  return b[i*4] + b[1+i*4]*256 + b[2+i*4]*256*256 + b[3+i*4]*256*256*256;
}

Rollsum.prototype.getDigestData = function(i){
